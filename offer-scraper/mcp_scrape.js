async (page) => {
  ...
  await page.goto('https://catalogodecursos.usfq.edu.ec/dashboard/home', { waitUntil: 'domcontentloaded', timeout: 60000 });
  ...
  await page.locator("button:has-text('Actualizar'), button:has-text('Update Courses')").first().click();
  ...
  while (true) {
    const raw = await page.evaluate(() => {
      ...
      for (const row of rows) {
        ...
        results.push({
          ...
          title, type: courseType, group_letters: groupLetters, paralelo,
    ...
    const nextBtn = page.locator("li.page-item:not(.disabled) a.page-link:has-text('Next')");
    if (await nextBtn.count() === 0) break;
    const lastNrc = allRows.length ? allRows[allRows.length - 1].nrc : '';
    await nextBtn.first().click();
    try {
      await page.waitForFunction(
        (nrc) => document.querySelector('table tbody tr td:nth-child(3)')?.innerText?.trim() !== nrc,
        lastNrc, { timeout: 15000 }
      );
  ...
  return { period_code: PERIOD_CODE, period: PERIOD_LABEL, count: allRows.length, courses: allRows };
}
